...
class CVUSADatasetTrain(Dataset):
    ...
    def __getitem__(self, index):
        ...
        n = int(torch.randint(0,4,(1,1)))
        width = query_img.shape[-1]
        limit_fov = 360
        if n == 0:
            limit_fov = 360
            noise = 0
            fov_label = torch.tensor([0])
        elif n == 1: 
            limit_fov = 360
            noise = 360
            fov_label = torch.tensor([1])    
        elif n == 2:
            limit_fov = 180
            noise = 360
            fov_label = torch.tensor([2])
        elif n == 3:
            limit_fov = 70
            noise = 360
            fov_label = torch.tensor([3])
        else:
            print("Data Error")
        ...
        #if limit_fov != 360:
        if noise == 360:
            j = torch.arange(0, query_img.shape[2])
            a = torch.rand(1)
            random_shift = int(a * width)
            grd_width  = int(limit_fov/360*width)
            query_img = query_img[:, :,((j - random_shift) % 768)[:grd_width]]       
        # two way for copy 1.repeat(n,n,n) 2.repeat_interleave(n,dim)
        if limit_fov==270:
            query_img = query_img.repeat(1,1,2)
            query_img = query_img[:,:,:width]
        elif limit_fov==180:
            query_img = query_img.repeat(1,1,2)
        elif limit_fov==90:
            query_img = query_img.repeat(1,1,4)
        elif limit_fov==70:
            query_img = query_img.repeat(1,1,6)
            query_img = query_img[:,:,:width]
        ...
        return query_img, reference_img, label, fov_label
...
class CVUSADatasetEval(Dataset):
    ...
    def __getitem__(self, index):
        ...
        width = img.shape[-1]
        if self.limit_fov == 360 and self.noise == 360:
    
            j = torch.arange(0, img.shape[2])
            a = torch.rand(1)
            random_shift = int(a * width)
           
            grd_width  = int(self.limit_fov/360*width)
            img =  img[:, :,((j - random_shift) % 768)[:grd_width]] 
        else:
            grd_width  = int(self.limit_fov /360*width)
            img = img[:,:,:grd_width]
        
        if self.limit_fov == 180:
            img = img.repeat(1,1,2)
        elif self.limit_fov == 270:
            img = img.repeat(1,1,2)
            img = img[:,:,:width]      
        elif self.limit_fov == 90 :
            img = img.repeat(1,1,4)
        elif self.limit_fov==70:
            img = img.repeat(1,1,6)
            img = img[:,:,:width]
