...
import gc
import copy
from ..trainer_cvusa_cvact import predict
...
def evaluate(config,
    ...
    print("\nExtract Features:")
    query_features, query_labels = predict(config, model, query_dataloader,mode=True)
    reference_features, reference_labels = predict(config, model, reference_dataloader,mode=False) 
    #query_features, query_labels = predict(config, model, query_dataloader,mode=True)
...
def calculate_scores(query_features, reference_features, query_labels, reference_labels, step_size=1000, ranks=[1,5,10]):
    ...
    for i in range(steps):
        ...
        similarity[start:end,:] = sim_tmp.cpu()
